...
[An example](https://git.ucd.gpn.gov.uk/dip/aws-internal-compute/blob/master/aws-cyi-infrastructure_network.tf) of this can be seen in the `internal-compute` VPC where a lot of our EMR clusters are deployed. 
...
This job will start an aws-cyi-infrastructure cluster. In order to make the cluster do what you want it to do, you can alter the following environment variables in the pipeline config and then run `aviator` to update the pipeline before kicking it off:
...
For stopping clusters, you can run the `stop-cluster` job to terminate ALL current `aws-cyi-infrastructure` clusters in the environment.
