resource "aws_security_group" "aws_cyi_infrastructure_master" {
  name                   = "aws_cyi_infrastructure Master"
  description            = "Contains rules for aws_cyi_infrastructure master nodes; most rules are injected by EMR, not managed by TF"
...
resource "aws_security_group" "aws_cyi_infrastructure_slave" {
  name                   = "${local.emr_cluster_name} Slave"
  description            = "Contains rules for aws_cyi_infrastructure slave nodes; most rules are injected by EMR, not managed by TF"
  revoke_rules_on_delete = true
  vpc_id                 = data.terraform_remote_state.internal_compute.outputs.vpc.vpc.vpc.id
  tags = {
...
resource "aws_security_group" "aws_cyi_infrastructure_common" {
  name                   = "${local.emr_cluster_name} Common"
  description            = "Contains rules for both aws_cyi_infrastructure master and aws_cyi_infrastructure slave nodes"
  revoke_rules_on_delete = true
  vpc_id                 = data.terraform_remote_state.internal_compute.outputs.vpc.vpc.vpc.id
  tags = {
...
resource "aws_security_group" "aws_cyi_infrastructure_emr_service" {
  name                   = "aws_cyi_infrastructure EMR Service"
  description            = "Contains rules for EMR service when managing the aws_cyi_infrastructure cluster; rules are injected by EMR, not managed by TF"
  revoke_rules_on_delete = true
  vpc_id                 = data.terraform_remote_state.internal_compute.outputs.vpc.vpc.vpc.id
  tags = {
    Name = "aws_cyi_infrastructure_emr_service"
  }
}
...
resource "aws_security_group_rule" "egress_https_to_vpc_endpoints" {
  ...
  from_port                = 443
  protocol                 = "tcp"
  security_group_id        = aws_security_group.aws_cyi_infrastructure_common.id
  to_port                  = 443
  type                     = "egress"
...
resource "aws_security_group_rule" "ingress_https_vpc_endpoints_from_emr" {
  description              = "Allow HTTPS traffic from aws_cyi_infrastructure"
  from_port                = 443
  protocol                 = "tcp"
  security_group_id        = data.terraform_remote_state.internal_compute.outputs.vpc.vpc.interface_vpce_sg_id
  to_port                  = 443
  type                     = "ingress"
  source_security_group_id = aws_security_group.aws_cyi_infrastructure_common.id
}
...
resource "aws_security_group_rule" "ingress_internet_proxy" {
  description              = "Allow proxy access from aws_cyi_infrastructure"
  type                     = "ingress"
  from_port                = 3128
  to_port                  = 3128
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "cyi_host_outbound_tanium_1" {
  description       = "CYI host outbound port 1 to Tanium"
  type              = "egress"
...
resource "aws_security_group_rule" "cyi_host_outbound_tanium_2" {
  description       = "CYI host outbound port 2 to Tanium"
  type              = "egress"
...
resource "aws_security_group_rule" "cyi_host_inbound_tanium_1" {
  description       = "CYI host inbound port 1 from Tanium"
  type              = "ingress"
  from_port         = var.tanium_port_1
  to_port           = var.tanium_port_1
  protocol          = "tcp"
...
resource "aws_security_group_rule" "cyi_host_inbound_tanium_2" {
  description       = "CYI host inbound port 2 from Tanium"
  type              = "ingress"
  from_port         = var.tanium_port_2
  to_port           = var.tanium_port_2
  protocol          = "tcp"
...
resource "aws_security_group_rule" "egress_aws_cyi_infrastructure_to_dks" {
...
resource "aws_security_group_rule" "ingress_to_dks" {
  provider    = aws.crypto
  description = "Allow inbound requests to DKS from aws_cyi_infrastructure"
  type        = "ingress"
  protocol    = "tcp"
  from_port   = 8443
  to_port     = 8443

  cidr_blocks = data.terraform_remote_state.internal_compute.outputs.cyi_subnet.cidr_blocks
...
resource "aws_security_group_rule" "emr_service_ingress_master" {
  ...
  to_port                  = 9443
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.aws_cyi_infrastructure_master.id
  security_group_id        = aws_security_group.aws_cyi_infrastructure_emr_service.id
}
...
resource "aws_security_group_rule" "emr_master_to_core_egress_tcp" {
  ...
  to_port                  = 65535
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.aws_cyi_infrastructure_slave.id
  security_group_id        = aws_security_group.aws_cyi_infrastructure_master.id
}
...
resource "aws_security_group_rule" "emr_core_to_master_egress_tcp" {
  ...
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.aws_cyi_infrastructure_master.id
  security_group_id        = aws_security_group.aws_cyi_infrastructure_slave.id
}
